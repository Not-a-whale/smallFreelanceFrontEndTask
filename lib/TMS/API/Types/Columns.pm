...
coerce 'EnumNull', from 'Str', via {
    if ($AUTO_GENERATE) {
        my @list = (
            'null',   'annually',  'bi-annually', 'quarterly', 'monthly', 'bi-monthly',
...
# ............................................................................
subtype 'EnumPublic', as 'Str', where {
    my %options = ('public' => 1, 'private' => 1);
    return exists $options{$_};
};

coerce 'EnumPublic', from 'Str', via {
    if ($AUTO_GENERATE) {
        my @list = ('public', 'private');
        return rand_enum(set => \@list);
    } else {
        s/(^\s+|\s+$)//g;
        s/\s+/ /g;
...
coerce 'EnumHourly', from 'Str', via {
    if ($AUTO_GENERATE) {
        my @list = (
            'hourly',  'salary',  'weekly',          'biweekly',
            'monthly', 'mileage', 'load percentage', 'flat rate'
        );
        return rand_enum(set => \@list);
    } else {
        s/(^\s+|\s+$)//g;
...
subtype 'Enum', as 'Str', where {
    my %options
        = ('' => 1, 'mr.' => 1, 'mrs.' => 1, 'ms.' => 1, 'dr.' => 1, 'sir' => 1, 'madam' => 1);
    return exists $options{$_};
};
